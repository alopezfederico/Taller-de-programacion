...
procedure Eliminar (var v: vector; var dimL: rango3; valorInferior, valorSuperior: rango1);
    ...
    Begin
        posInferior:= BuscarPosicion (v, dimL, valorInferior); // = 3
        if (posInferior <> 0)
        then begin
                posSuperior:= BuscarPosicionDesde (v, dimL, posInferior, valorSuperior); // = 6
  
                {Escribir el código correspondiente para hacer el corrimiento y disminuir la dimensión lógica}
                if not(posSuperior=dimL)then
                    begin
                    salto:= dimL - posSuperior;
                    for i:= 0 to salto do
                        v[posInferior + i]:= v[posSuperior + i + 1];
                    end;
    ...
    begin
        while(L<>nil)do
            begin
                ...
                writeln('Cantidad de productos vendidos: ', L^.dato.cantTotal);
