...
a. Un módulo recursivo que retorne un vector de a lo sumo 15 números enteros “random” mayores a 10 y menores a 155 (incluidos ambos). La carga finaliza con el valor 20.                 HECHO

b. Un módulo no recursivo que reciba el vector generado en a) e imprima el contenido del vector.                                                                                         HECHO

c. Un módulo recursivo que reciba el vector generado en a) e imprima el contenido del vector.                                                                                            HECHO

d. Un módulo recursivo que reciba el vector generado en a) y devuelva la suma de los valores pares contenidos en el vector.                                                              HECHO
...
g. Un módulo que reciba el vector generado en a) e imprima, para cada número contenido en el vector, sus dígitos en el orden en que aparecen en el número. Debe implementarse un
...
procedure ImprimirDigitos (v: vector; dimL: integer);
    ...
    begin    
        if (dimL <> 0) then
            begin
                write('Numero: ',v[dimL],'. Digitos: ');
                Digitos(v[dimL]);write(' | ');
                writeln();
                dimL:= dimL - 1;
...
randomize;
  ...
  encontre:= BuscarValor(v, dimL, valor);   writeln; writeln;
