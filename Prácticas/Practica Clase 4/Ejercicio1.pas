...
De cada producto deben quedar almacenados
 su código, 
 la cantidad total de unidades vendidas y 
 el monto total. 

 De cada venta se cargan 
 código de venta, 
...
procedure ModuloA (var a: arbol);
 ...
 de unidades vendidas y el monto total. }
    ...
    var 
        unaVenta: venta;  
    Begin
        writeln;writeln ('----- Ingreso de ventas y armado de arbol de productos ----->');writeln;
        a:= nil;
...
procedure ModuloB (a: arbol);
 { Imprima el contenido del árbol ordenado por código de producto.}
    ...
    begin
        writeln; writeln('----- Modulo B ----->'); writeln;
        if ( a = nil) then
             writeln ('Arbol vacio')
        else
             ImprimirArbol (a);
...
procedure ModuloC (a: arbol);
    ...
    var 
        menorCodigo: integer;
    begin
        writeln; writeln ('----- Modulo C ----->'); writeln;
        write ('Menor codigo de producto: '); writeln;
        menorCodigo:= ObtenerMinimo (a);
        if (menorCodigo = 9999) then  
            writeln ('Arbol vacio') 
        else 
            begin
                writeln; writeln ('El codigo menor es ', menorCodigo); writeln;
            end;
        writeln; writeln ('-----------------------------------------------'); writeln;
    end;
        ...
        begin
            if (a=nil) then
                ...
                if (a^.dato.codigo < cod) then
                    ...
                    CantidadDeCodigosMenores:= 0 + CantidadDeCodigosMenores(a^.HI,cod)
        end;
...
{ Contenga un módulo que reciba la estructura generada en el punto a y dos códigos de producto y 
    ...
    begin
        if (a=nil) then
            ...
            if (a^.dato.codigo > codigo1) and (a^.dato.codigo < codigo2) then
                ...
                if not(a^.dato.codigo > codigo1) then
                    ObtenerMontoTotalEntreDosCodigos:= 0 + ObtenerMontoTotalEntreDosCodigos(a^.HD,codigo1,codigo2)
            else
                if not(a^.dato.codigo < codigo2) then
                    ObtenerMontoTotalEntreDosCodigos:= 0 + ObtenerMontoTotalEntreDosCodigos(a^.HI,codigo1,codigo2);
    end;
    ...
    begin
        ...
        if (montoTotal = 0) then 
          writeln ('No hay codigos entre ', codigo1, ' y ', codigo2)
            ...
            begin
                writeln;
                writeln ('El monto total entre el codigo', codigo1, ' y el codigo : ', codigo2, ' es: ', montoTotal:6:2); 
                writeln;
            end;
        writeln;
        writeln ('-----------------------------------------------');
